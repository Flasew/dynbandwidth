...
int main(int argc, char * argv[]) {
  ...
  char buf[1024] = {};
  ignore = system("/sbin/tc qdisc add dev macvlan0 root handle 1:0 tbf rate 5.5Gbit burst 48750b lat 100ms");
  snprintf(buf, 1024, "tc qdisc add dev macvlan0 parent 1:1 handle 10: netem limit %llu", ql);
  ignore = system(buf);
  memset(buf, 0, 1024);
  ignore = system("/sbin/tc qdisc add dev macvlan1 root handle 1:0 tbf rate 5.5Gbit burst 48750b lat 100ms");
  snprintf(buf, 1024, "tc qdisc add dev macvlan1 parent 1:1 handle 10: netem limit %llu", ql);
  ignore = system(buf);
